...
/*
pokemon - array name
[index] - each array element is an object
.id , .name , .types - keys for each object
.types [0] [1] - each index is a type
*/
...
for (var i = 0; i < pokemon.length; i++) {
    if ( pokemon[i].id % 3 == 0) {
        console.log("Name:", pokemon[i].name, pokemon[i]);
    }
}
...
for (var i = 0; i < pokemon.length; i++) {
    if ( pokemon[i].types == "poison") {
        console.log("Name:", pokemon[i].name);
    }
}
...
for (var i = 0; i < pokemon.length; i++) {
    if ( pokemon[i].types.length > 1 && pokemon[i].types[1] == "flying") {
        console.log("Name:", pokemon[i].name, " -- First type:", pokemon[i].types[0]);
    }
}
...
for(var i=0; i<pokemon.length; i++) {
    if((pokemon[i].types.length == 1) && (pokemon[i].types[0] == "poison")) {
        for(var p = pokemon[i].name.length - 1; p >= 0; p--){
            console.log(pokemon[i].name[p])
        }
    }
}
